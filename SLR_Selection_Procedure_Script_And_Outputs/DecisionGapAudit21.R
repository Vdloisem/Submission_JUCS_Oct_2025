...
# ----------------------------- User parameters --------------------------------
in_csv   <- "slr_outputs_sq21/SQ2.1_with_ReasonCode.csv"  
out_dir  <- "decision_gap_outputs_SQ2.1"                   
...
gap_summary <- tibble(
  ...
  exc_max_margin_p95 = if (length(exc_delta)) unname(quantile(exc_delta, 0.95, na.rm = TRUE)) else NA_real_
)
...
ggsave(file.path(out_dir, "plot_decision_margin.png"), p_margin, width = 7, height = 4.5, dpi = 150)
...
if (!is.null(roc_obj)) {
  p_roc <- ggplot(data.frame(
    ...
    theme_minimal(base_size = 12)
  ggsave(file.path(out_dir, "plot_roc.png"), p_roc, width = 6, height = 5, dpi = 150)
}
...
if (nrow(calib_q) > 0) {
  p_cal_q <- ggplot(calib_q, aes(bin_center, empirical_include)) +
    ...
    theme_minimal(base_size = 12)
  ggsave(file.path(out_dir, "plot_calibration_quantiles.png"), p_cal_q, width = 6.5, height = 4.5, dpi = 150)
}
...
if (nrow(calib_f) > 0) {
  p_cal_f <- ggplot(calib_f, aes(center, empirical_include)) +
    ...
    theme_minimal(base_size = 12)
  ggsave(file.path(out_dir, "plot_calibration_fixedbins.png"), p_cal_f, width = 7, height = 4.5, dpi = 150)
}
